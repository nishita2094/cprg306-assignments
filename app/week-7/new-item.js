...
export default function NewItem({ onAddItem }) {
  ...
  const handleSubmit = (event) => {
    ...
    onAddItem(item); // ✅ pass item to parent handler
  ...
  if (count >= 20) buttonStyles += " opacity-50 cursor-not-allowed";

  let buttonStylesMinus = "text-2xl mt-4 bg-blue-500 text-white p-4 ml-5 rounded";
  if (count <= 1) buttonStylesMinus += " opacity-50 cursor-not-allowed";
